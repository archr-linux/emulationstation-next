...
{
	return executeScript("archr-config overscan " + std::string(enable ? "enable" : "disable"));
}
...
std::pair<std::string, int> ApiSystem::updateSystem(const std::function<void(const std::string)>& func)
{
	...
	std::string updatecommand = "archr-update";
...
std::pair<std::string, int> ApiSystem::scrape(BusyComponent* ui) 
{
	...
	FILE* pipe = popen("archr-scraper", "r");
	if (pipe == nullptr)
...
bool ApiSystem::canUpdate(std::vector<std::string>& output) 
{
	...
	FILE *pipe = popen("archr-update check", "r");
	if (pipe == NULL)
		return false;
...
{
	return executeScript("archr-bluetooth enable 2>&1 >/dev/null");
}

bool ApiSystem::disableBluetooth()
{
	return executeScript("archr-bluetooth disable");
}

void ApiSystem::startBluetoothLiveDevices(const std::function<void(const std::string)>& func)
{
	executeScript("archr-bluetooth live_devices", func);
}

void ApiSystem::stopBluetoothLiveDevices()
{
	executeScript("archr-bluetooth stop_live_devices");
}
...
{
	return executeScript("archr-bluetooth trust input", func).second == 0;
}

std::vector<std::string> ApiSystem::getPairedBluetoothDeviceList()
{
	return executeEnumerationScript("archr-bluetooth list");
}

std::vector<std::string> ApiSystem::getAvailableStorageDevices() 
{
	return executeEnumerationScript("archr-config storage list");
}
...
{
	return executeEnumerationScript("archr-sync list");
}
...
{
	return executeEnumerationScript("archr-info --full");
}
...
std::string ApiSystem::getRumblePath()
{
	...
	for (auto file : files)
	{
		...
		if ((Utils::String::toLower(file).find("/archr") != std::string::npos) && (joypadPath.empty()))
			joypadPath = file;
	}
...
{
	return executeScript("archr-config forgetBT");
}
...
std::vector<BatoceraBezel> ApiSystem::getBatoceraBezelsList()
{
	...
	auto lines = executeEnumerationScript("archr-es-thebezelproject list");
	for (auto line : lines)
	{
...
bool ApiSystem::isScriptingSupported(ScriptId script)
{
	...
	case ApiSystem::BLUETOOTH:
		executables.push_back("archr-bluetooth");
		break;
	case ApiSystem::RESOLUTION:
		executables.push_back("archr-resolution");
		break;
	case ApiSystem::BIOSINFORMATION:
		executables.push_back("archr-systems");
		break;
	...
	case ApiSystem::THEBEZELPROJECT:
		executables.push_back("archr-es-thebezelproject");
		break;		
	...
	case ApiSystem::BACKUP:
		executables.push_back("archr-sync");
		break;
	...
	case ApiSystem::UPGRADE:
		executables.push_back("archr-update");
		break;
